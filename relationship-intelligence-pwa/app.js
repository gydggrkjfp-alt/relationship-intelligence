const STORAGE_KEY='relationship_intelligence_pwa_v11';
...
function fillMe(){let me=state.me||defaultMe();$('myName').value=me.name||'';$('myPhilosophy').value=me.philosophy||'';needDefs.forEach(([k])=>{let v=me.needs?.[k]??5;$(`need_${k}`).value=v;$(`need_${k}_value`).textContent=v});roleDefs.forEach(([k])=>{let el=$(`role_${k}`);if(el){let v=me.roles?.[k]??5;el.value=v;$(`role_${k}_value`).textContent=v}});tendencyDefs.forEach(([k])=>$(`t_${k}`).checked=!!me.tendencies?.[k]);updateMyReadout()}
...
function collectMe(){let me=state.me||defaultMe();me.name=$('myName').value;me.philosophy=$('myPhilosophy').value;needDefs.forEach(([k])=>me.needs[k]=Number($(`need_${k}`).value));me.roles=me.roles||{};roleDefs.forEach(([k])=>{let el=$(`role_${k}`);if(el)me.roles[k]=Number(el.value)});tendencyDefs.forEach(([k])=>me.tendencies[k]=$(`t_${k}`).checked);state.me=me}
...
function renderEcosystem(){
  ...
  renderNutrients();renderDeficiencies();renderCultivation();renderRoleOutput();drawEcosystemCanvas();
}
...
function init(){try{renderSliders();currentProfile();bindEvents();fillForm();fillMe();renderProfiles();updateQuickLabels();updateQuickCardLabels();safeUpdate();renderCards();renderEcosystem();saveState();$('status').textContent='App loaded. Autosave active.'}catch(e){$('status').textContent='Startup error: '+e.message;console.error(e)}}
