const STORAGE_KEY='relationship_intelligence_pwa_v9_2';
...
const categoryDefs={
  Romantic:[['commitment','Commitment readiness','Is this person oriented toward real partnership?'],['repair','Repair capacity','Can they talk through conflict and reconnect?'],['values','Shared values','Do life direction and values fit?'],['affection','Affection fit','Does affection style feel mutually satisfying?'],['availability','Emotional availability','Are they actually available, not just attractive?']],
  Friend:[['mutuality','Mutuality','Does friendship effort flow both ways?'],['trust','Trustworthiness','Can you trust them with personal information?'],['fun','Enjoyment','Do you actually enjoy time together?'],['support','Supportiveness','Do they celebrate wins and show up when needed?'],['lowDrama','Low drama','Does this friendship avoid needless turbulence?']],
  Work:[['competence','Competence','Do they do their job well enough to reduce friction?'],['reliability','Reliability','Do they follow through?'],['clarity','Communication clarity','Are expectations and messages clear?'],['fairness','Fairness','Do they treat you and others fairly?'],['politics','Low politics','Do they avoid gossip, credit games, and status traps?']],
  Family:[['boundaries','Boundary respect','Can they respect limits?'],['predictability','Predictability','Do interactions follow a predictable pattern?'],['obligation','Obligation load','How much duty or guilt pressure exists?'],['repair','Repair capacity','Can conflict be repaired?'],['exposure','Sustainable exposure','Can you be around them without major emotional cost?']],
  Boundary:[['risk','Boundary risk','How risky is closeness?'],['distance','Distance clarity','Is the proper distance clear?'],['manipulation','Manipulation pressure','Do they pressure, guilt, or pull you in?'],['safety','Emotional safety','Can you interact without destabilization?'],['exit','Ease of exit','Can you disengage cleanly?']],
  General:[['ease','Ease','Does interaction feel basically easy?'],['trust','Trust','Can you trust the person?'],['respect','Respect','Is there mutual respect?'],['benefit','Benefit','Does this connection provide real value?'],['cost','Low cost','Is emotional cost manageable?']]
};
const energyDefs=[['gain','Energy gained','Does this interaction give energy, warmth, motivation, or connection?'],['drain','Energy drained','Does this interaction leave you tired, anxious, or depleted?'],['rumination','Rumination afterward','Do you keep replaying the interaction?'],['grounding','Grounding effect','Do you feel more stable and clear afterward?']];
...
function loadState(){try{let raw=localStorage.getItem(STORAGE_KEY);if(raw)return JSON.parse(raw);for(let key of ['relationship_intelligence_pwa_v9','relationship_intelligence_pwa_v8','relationship_intelligence_pwa_v7','relationship_intelligence_pwa_v6','relationship_intelligence_pwa_v5','relationship_intelligence_pwa_v4','relationship_intelligence_pwa_v3','relationship_intelligence_pwa_v2','relationship_intelligence_pwa_v1']){let old=localStorage.getItem(key);if(old)return migrate(JSON.parse(old));}}catch(e){}return{currentId:null,profiles:[],me:defaultMe()}}
function migrate(s){s.me=s.me||defaultMe();s.profiles=(s.profiles||[]).map(p=>{p={...p,evidence:p.evidence||'',interpretation:p.interpretation||'',hesitation:p.hesitation||'',socialNotes:p.socialNotes||'',desiredOutcome:p.desiredOutcome||'Explore slowly',rtype:p.rtype==='Potential romantic partner'?'Romantic prospect':(p.rtype||'Romantic prospect')};p.social=p.social||{};p.category=p.category||{};p.energy=p.energy||{};p.story=p.story||'';p.respect=p.respect||{};socialDefs.forEach(([k,,,,good])=>{if(p.social[k]===undefined)p.social[k]=good?5:3});respectDefs.forEach(([k])=>{if(p.respect[k]===undefined)p.respect[k]=5});Object.values(categoryDefs).flat().forEach(([k])=>{if(p.category[k]===undefined)p.category[k]=5});energyDefs.forEach(([k])=>{if(p.energy[k]===undefined)p.energy[k]=k==='drain'||k==='rumination'?3:5});p.snapshots=p.snapshots||[];return p});return s}
...
function blankProfile(){let p={id:uid(),name:'',rtype:'Romantic prospect',met:'Soccer',desiredOutcome:'Explore slowly',socialNotes:'',impression:'',evidence:'',interpretation:'',hesitation:'',notes:'',green:{},risk:{},respect:{},social:{},category:{},energy:{},story:'',snapshots:[]};greenDefs.forEach(([k])=>p.green[k]=5);riskDefs.forEach(([k])=>p.risk[k]=3);respectDefs.forEach(([k])=>p.respect[k]=5);socialDefs.forEach(([k,,,,good])=>p.social[k]=good?5:3);Object.values(categoryDefs).flat().forEach(([k])=>p.category[k]=5);energyDefs.forEach(([k])=>p.energy[k]=k==='drain'||k==='rumination'?3:5);return p}
...
function renderCategorySliders(){
  if(!$('categorySliders'))return;
  let p=currentProfile(), cat=cardCategoryForAdaptive(p), defs=categoryDefs[cat]||categoryDefs.General;
  $('categorySliders').innerHTML=`<div class="categoryNotice"><b>${cat} lens</b><div class="small">These qualities are weighted for this card type.</div></div>`+defs.map(([k,l,h])=>sliderHTML('category',k,l,h,p.category?.[k]??5,'Low','High',true)).join('');
}
...
function fillForm(){let p=currentProfile();$('profileTitle').textContent='Profile: '+(p.name||'Untitled');['name','rtype','met','desiredOutcome','socialNotes','impression','evidence','story','interpretation','hesitation','notes'].forEach(id=>$(id).value=p[id]||'');if(!p.rtype)$('rtype').value='Romantic prospect';if(!p.met)$('met').value='Soccer';greenDefs.forEach(([k])=>{let v=p.green?.[k]??5;$(`green_${k}`).value=v;$(`green_${k}_value`).textContent=v});riskDefs.forEach(([k])=>{let v=p.risk?.[k]??3;$(`risk_${k}`).value=v;$(`risk_${k}_value`).textContent=v});respectDefs.forEach(([k])=>{let v=p.respect?.[k]??5;$(`respect_${k}`).value=v;$(`respect_${k}_value`).textContent=v});socialDefs.forEach(([k,,,,good])=>{let v=p.social?.[k]??(good?5:3);$(`social_${k}`).value=v;$(`social_${k}_value`).textContent=v});renderCategorySliders();(categoryDefs[cardCategoryForAdaptive(p)]||categoryDefs.General).forEach(([k])=>{let el=$(`category_${k}`);if(el){let v=p.category?.[k]??5;el.value=v;$(`category_${k}_value`).textContent=v}});if($('energySliders'))energyDefs.forEach(([k])=>{let el=$(`energy_${k}`);if(el){let v=p.energy?.[k]??(k==='drain'||k==='rumination'?3:5);el.value=v;$(`energy_${k}_value`).textContent=v}})}
...
function collectForm(){let p=currentProfile();['name','rtype','met','desiredOutcome','socialNotes','impression','evidence','story','interpretation','hesitation','notes'].forEach(id=>p[id]=$(id).value);greenDefs.forEach(([k])=>p.green[k]=Number($(`green_${k}`).value));riskDefs.forEach(([k])=>p.risk[k]=Number($(`risk_${k}`).value));respectDefs.forEach(([k])=>p.respect[k]=Number($(`respect_${k}`).value));socialDefs.forEach(([k])=>p.social[k]=Number($(`social_${k}`).value));(categoryDefs[cardCategoryForAdaptive(p)]||categoryDefs.General).forEach(([k])=>{let el=$(`category_${k}`);if(el)p.category[k]=Number(el.value)});energyDefs.forEach(([k])=>{let el=$(`energy_${k}`);if(el)p.energy[k]=Number(el.value)})}
...
function updateEvidenceStory(p,m){
  if(!$('evidenceStoryOutput'))return;
  let ev=(p.evidence||'').trim(), st=(p.story||'').trim();
  let msg='Add evidence and story separately to reduce projection.';
  if(ev.length>20&&st.length>20)msg='Good: you separated observable data from interpretation. Now check whether the story is supported by repeated evidence.';
  else if(st.length>30&&ev.length<15)msg='Story-heavy update: your interpretation may be outrunning the facts.';
  else if(ev.length>30&&st.length<15)msg='Evidence-heavy update: useful. Add the story only if you notice yourself making assumptions.';
  $('evidenceStoryOutput').innerHTML=`<p><b>Signal check:</b> ${msg}</p>`;
}
function updateTrend(p){
  if(!$('trendOutput'))return;
  let snaps=p.snapshots||[];
  if(snaps.length<2){$('trendOutput').innerHTML='<p><b>Trend:</b> not enough snapshots yet. Add at least two quick updates.</p>';return;}
  let first=snaps[0],last=snaps[snaps.length-1];
  let dp=last.peace-first.peace,dr=last.respect-first.respect,dc=last.compat-first.compat;
  $('trendOutput').innerHTML=`<p><b>Trend:</b> Peace ${dp>=0?'+':''}${dp}, Respect ${dr>=0?'+':''}${dr}, Compatibility ${dc>=0?'+':''}${dc} since first snapshot.</p><p>${dp<0||dr<0?'Watch deterioration; something is costing more over time.':'Current direction is stable or improving.'}</p>`;
}
function updateEnergy(p,m){
  if(!$('energyOutput'))return;
  let e=m.energy;
  $('energyOutput').innerHTML=`<p><b>Energy net:</b> <span class="energyNet">${e>=0?'+':''}${e}</span></p><p>${e>=15?'This relationship appears energizing.':e>=0?'Energy cost looks manageable.':e>-20?'This relationship has a noticeable emotional cost.':'High emotional drain. Consider boundaries or reduced exposure.'}</p>`;
}
function updateArchetype(p,m){
  if(!$('archetypeOutput'))return;
  let arch='Mixed / still learning', forecast='Collect more evidence before making big moves.';
  if(m.peaceIndex>=70&&m.respectIndex>=70&&m.energy>=0){arch='Stable builder';forecast='If this pattern holds, increased trust is plausible.'}
  else if(p.green.attraction>=8&&m.peaceIndex<50){arch='High pull / low peace';forecast='If unchanged, emotional volatility may rise with investment.'}
  else if(m.respectIndex<45){arch='Respect deficit';forecast='If unchanged, resentment or humiliation risk increases.'}
  else if(m.energy< -15){arch='Emotional drain';forecast='If unchanged, reduce exposure or narrow the relationship context.'}
  else if(p.risk.chaos>=7){arch='Warm but chaotic';forecast='If unchanged, intensity may substitute for security.'}
  $('archetypeOutput').innerHTML=`<p><b>Archetype:</b> ${arch}</p><p><b>Forecast:</b> ${forecast}</p>`;
}
...
function init(){try{renderSliders();currentProfile();bindEvents();fillForm();bindCategoryInputs();fillMe();renderProfiles();updateQuickLabels();safeUpdate();renderCards();saveState();$('status').textContent='App loaded. Autosave active.'}catch(e){$('status').textContent='Startup error: '+e.message;console.error(e)}}
