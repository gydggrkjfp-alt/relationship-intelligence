const STORAGE_KEY='relationship_intelligence_pwa_v9';
...
function loadState(){try{let raw=localStorage.getItem(STORAGE_KEY);if(raw)return JSON.parse(raw);for(let key of ['relationship_intelligence_pwa_v8','relationship_intelligence_pwa_v7','relationship_intelligence_pwa_v6','relationship_intelligence_pwa_v5','relationship_intelligence_pwa_v4','relationship_intelligence_pwa_v3','relationship_intelligence_pwa_v2','relationship_intelligence_pwa_v1']){let old=localStorage.getItem(key);if(old)return migrate(JSON.parse(old));}}catch(e){}return{currentId:null,profiles:[],me:defaultMe()}}
...
function renderCategorySliders(){
  ...
  let cat=cardCategoryForAdaptive(currentProfile());
  let defs=categoryDefs[cat]||categoryDefs.General;
  $('categorySliders').innerHTML=`<div class="categoryNotice"><b>${cat} lens</b><div class="small">These qualities are weighted for this card type.</div></div>`+defs.map(([k,l,h])=>sliderHTML('category',k,l,h,5,'Low','High',true)).join('');
}
...
function updateEvidenceStory(p,m){
  ...
  if(ev.length>20&&st.length>20)msg='Good: you separated observable data from interpretation. Now check whether the story is actually supported by repeated evidence.';
