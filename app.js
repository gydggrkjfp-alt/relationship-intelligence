...
function expertDisplay343(name){
 const map={
  ...
  'Tracy Vaillancourt female social dynamics lens':'Tracy Vaillancourt dynamics lens',
...
function expertProfile343(name,row={}){
 const profiles={
  ...
  'Tracy Vaillancourt female social dynamics lens':{avatar:'TV',kind:'Female peer dynamics / research',lineage:'Modeled voice based on Tracy Vaillancourt research and Modern Wisdom discussion: indirect aggression, sexy-peer intolerance, social comparison, jealousy, gossip, exclusion, and reputation punishment among women.',bio:'Best when women are trying to understand sisterhood pressure, gossip, attractiveness policing, sexual reputation, envy, or whether advice from other women is protective or competitive.'},
...
function inferredIssueType380(raw,crux=''){
 ...
 if(/female intra|intrasexual|women compete|female competition|women policing women|police women|sisterhood|gossip|reputation|ostrac|exclude|friend pressure|girlfriends|girl friends|mate choice|jealous women|jealousy|envy|social comparison|compare herself|comparison|attractive woman|sexy woman|sexy peer|sexy peers|dress down|slut|slut-sham|sexual shame|indirect aggression|social rivalry|peer pressure|sabotage|stay single|friends.*single|friends.*dump|women.*pressure/.test(t))return'social_rivalry';
 if(/dont chase|don't chase|left on read|left on red|option|plan b|backup|job offer|beta corp|applicant|interview|overpursu|overcommit|happy hunting|hunter|hunt|fisherman|fishing|conditional yes|lazy dating|date more intentionally|dating more intentionally|date intentionally|dating intentionally|choose a man|choose men|choose better|worth my time|actually worth|who is worth|character.*direction|direction.*character|safety.*attraction|attraction.*safety|long-term fit|long term fit|reacting to whoever pursues|whoever pursues me|men.*attention|attention from men|find a man|find men|get a boyfriend|get a husband|how do i date|how to date|meet men|women looking for men|woman looking for a man|female looking for a man|husband material|boyfriend material/.test(t))return'dating_strategy';
 ...
 if(/hot and cold|mixed signals|mixed signal|inconsistent over text|inconsistent text|inconsistent communication|warm in person|interested in person|slow to reply|slow reply|slow replies|pull back|pulling back|chase him|chase her|do not want to chase|don't want to chase|dont want to chase|miss a real opportunity|testing me|test me|indirect|passive|hesitat/.test(t))return'hot_cold';
...
function situationTitle380(raw,stage,crux,issueType=''){
 ...
 if(/female intra|intrasexual|women compete|female competition|women policing women|police women|sisterhood|gossip|reputation|ostrac|exclude|friend pressure|girlfriends|girl friends|mate choice|jealous women|jealousy|envy|social comparison|compare herself|comparison|attractive woman|sexy woman|sexy peer|sexy peers|dress down|slut|slut-sham|sexual shame|indirect aggression|social rivalry|peer pressure|sabotage|stay single|friends.*single|friends.*dump|women.*pressure/.test(t))return'social rivalry / reputation';
...
function geniusQuestion380(raw,stage,crux,people){
 ...
 if(crux==='behavior_change')return 'What concrete behavior should change, and how would I know it is actually sticking?';
	...
	function isWomenWantPrompt380(raw){
	 return /what (do )?women want|relationship opportunity|understand women|why women|why she|why girls|female behavior|from her side|hot and cold|mixed signals|mixed signal|inconsistent over text|inconsistent text|inconsistent communication|warm in person|interested in person|slow to reply|slow reply|slow replies|do not want to chase|don't want to chase|dont want to chase|miss a real opportunity|hesitat|indirect|passive|test me|testing me|pull back|care for me|wants to care|taken out|contribution|contribute|useful role|put her to work|overinvest|over-invest|ferryboat|besides dates|besides attention|besides sex/i.test(String(raw||''));
	}
	...
	function orionTurn380(raw,stage,crux,people,slot=0){
	 ...
	 const understand=/what women want|understand women|why women|why she|why girls|female behavior|mixed signals|mixed signal|inconsistent over text|inconsistent text|inconsistent communication|warm in person|interested in person|slow to reply|slow reply|slow replies|do not want to chase|don't want to chase|dont want to chase|miss a real opportunity|hesitat|indirect|passive|test me|testing me|pull back|hot and cold/.test(t);
...
function voiceCards380(row,cards=[]){
 ...
 const isOwnedVoiceCard=card=>id(card).startsWith('orion')||id(card).startsWith('kait')||id(card).startsWith('mw-alison')||id(card).startsWith('mw-suzanne')||id(card).includes('vaillancourt');
 ...
 else if(n.includes('Vaillancourt')||n.includes('Tracy'))filtered=pool.filter(c=>id(c).includes('vaillancourt')||/vaillancourt|sexy-peer|sexy peers|indirect aggression|social comparison|jealousy|envy|sisterhood|reputation|ostracism|female-intrasexual/.test(tags(c)));
...
function chatKind380(raw,issueType='',crux=''){
 ...
 if(issueType==='hot_cold'||/hot and cold|mixed signals|mixed signal|inconsistent over text|inconsistent text|inconsistent communication|warm in person|interested in person|slow to reply|slow reply|slow replies|pull back|pulling back|chase him|chase her|do not want to chase|don't want to chase|dont want to chase|miss a real opportunity|testing me|test me|indirect|passive|hesitat/.test(t))return'hot_cold';
...
function chatTextForVoice380(voice,kind,raw,stage,crux,people){
 const earlyMan=stage==='early'&&people?.user==='man'&&people?.target==='woman';
 const target=people?.target==='man'
  ?{subj:'he',obj:'him',poss:'his',person:'man'}
  :people?.target==='woman'
   ?{subj:'she',obj:'her',poss:'her',person:'woman'}
   :{subj:'they',obj:'them',poss:'their',person:'person'};
 const v=voice;
 const byKind={
  digital_boundary:{
  ...
  social_rivalry:{
   ...
   wild:'From a Vaillancourt-style lens, look for the mechanism: social comparison creates jealousy or threat, and then the group may use gossip, exclusion, sexual shame, or “concern” to regulate the woman who became too visible. The answer is not paranoia; it is discrimination. Keep women who give concrete safety evidence, and discount pressure that mostly makes you smaller, colder, less feminine, or less able to choose the life you actually want.'
  },
  contribution:{
  ...
  dating_strategy:{
   orion:/dont chase|don't chase|left on read|left on red|option|job offer|beta corp|overpursu|overcommit/i.test(raw)?`Do not become the unconditional job offer: already committed, endlessly available, and waiting with no cost to ${target.obj}. Make a clean invitation, keep your life and options real, and let reciprocal effort earn more access; otherwise you train yourself into the fallback role.`:people?.user==='woman'&&people?.target==='man'?'Do not date from random attention. Attention from men is not the same as selection, because attention only proves that someone wants access. Pick men whose life, direction, and values you might actually want, do enough homework before the date, then show up as a conditional yes if you accept: warm, open, and still free to revoke access if his behavior fails the standard.':'Men often fish by creating broad opportunity and selecting from women who choose back; women can hunt more specifically. Build a legible life, cast clean invitations, and stop trying to convert chronic maybe energy into devotion.',
   kait:people?.user==='woman'&&people?.target==='man'?'A man worth selecting should show capacity before you reward him with more access: direction, composure, follow-through, social respect, and the ability to handle your no. Part of why women want what they want is not always conscious: safety, status, resourcefulness, confidence, and peer judgment all blend into the feeling of attraction or hesitation. So do not only ask “am I wanted?” Ask whether your body relaxes around his competence, whether other women’s opinions are distorting your read, and whether his behavior makes respect and long-term attraction easier.':'If women are hunting, they are reading capacity: direction, composure, resources, fitness, standards, and whether your life is worth entering. Your job is to make capacity visible through behavior, not to explain your value until she believes it.',
   amy:people?.user==='woman'&&people?.target==='man'?'Warmth is not the same as surrender. Let him feel that you are pleased to be there if you chose the date, but keep your standards clear, kind, and appreciable. From an Alison-style lens, men often respond well when they know what succeeds with you: appreciation for integrity, clarity when something does not work, and enough respect that he feels invited to rise rather than cross-examined. A good man should feel wanted and respected, but he should also know your pace, safety, and dignity matter.':'Make initiative easy to receive. Lead with a clear plan, appreciate useful movement from her, and create a relationship channel where care can go both ways. If she has been hunting you, she should be able to enter your life with warmth, not just consume attention.',
   wild:'Courtship is selection under small pressure. Look for manners, follow-through, respect around pace, and whether the person becomes more honorable when desire appears. Attraction matters, but character decides whether more access is wise, and the social environment can either sharpen or contaminate that judgment.'
  },
  hot_cold:{
   orion:`Hot-cold behavior may be calibration, uncertainty, or low investment. Do not chase every temperature change; that rewards instability. Make one clear plan, keep your life intact, and let ${target.poss} reciprocal effort tell you whether this is real.`,
   kait:`Your posture matters here. If warmth makes you over-invest and distance makes you plead, ${target.subj} learns that your center is outside you. Stay composed, make a clear bid, and do not promote mixed signals into a privileged position.`,
   amy:`Do not punish uncertainty, but do ask for enough consistency to feel respected. A gentle sentence works better than a prosecution: “I like this when it feels mutual; I do not do well with hot and cold.” Then observe whether ${target.subj} can meet you.`,
   wild:`The answer is in the pattern after clarity. If ${target.subj} becomes steadier, good. If ambiguity continues to buy your attention, step back.`
  },
  sex_pacing:{
  ...
  grounding:{
   orion:`This is not a situation to solve by extracting more certainty from ${target.obj}. Rebuild the larger context: friends, body, faith or values, work, and a life ${target.subj} could enter without becoming the center of gravity. Attraction is easier to handle when your life is not waiting for one person to validate it.`,
...
function wildcardVoice380(kind,raw,stage,crux){
 if(kind==='social_rivalry')return{key:'Tracy Vaillancourt female social dynamics lens',name:'Tracy Vaillancourt dynamics lens',avatar:'TV',kind:'Female peer dynamics / research',voice:'wild'};
